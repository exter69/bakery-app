...
## 3. Test accounts (in-memory seed data)

From `cmd/server/seed.go`. Only valid in in-memory mode; with Postgres you must register fresh (and see MA-62).

| Username | Password | Role | Notes |
|----------|----------|------|-------|
| `admin` | `admin123` | 0 Admin | Reaches `/dashboard` |
| `baker_jean` | `baker123` | 1 Seller | Owns `bakery-1` (La Boulangerie du Coin) + `bakery-3` (Le Fournil de Max) |
| `baker_marie` | `baker123` | 1 Seller | Owns `bakery-2` (Mie et Beurre) |
| `alice` | `customer123` | 2 Customer | Has favorites + seeded orders |
| `bob` | `customer123` | 2 Customer | — |
| — | — | 3 Business | **No seed user and no register-UI path — see TC-AUTH-11** |

Seed bakeries: `bakery-1/2/3`. Seed products: `prod-1-1`…`prod-3-4` (prices in cents, e.g. Croissant 150 = €1.50).
...
| TC-AUTH-05 | — | Register duplicate username (`alice`) | Backend 409 → error surfaced, no crash |
...
| TC-AUTH-09 | Logged out | `/login` with `alice`/`customer123` | Login, redirect (customer → `/` or `from`) |
...
| TC-ORD-04 | Logged in as `alice` | Add items (delivery mode), set schedule, submit order | `createOrder` called; response `paymentUrl` → `window.location` redirect to payment |
...
| TC-HIS-01 | `alice` | `/history` | Past orders (20/pg), status badges, filter/sort |
| TC-HIS-02 | `alice` | Click "Re-order" on a past order | `storeReorderData` → routes to bakery with cart prefilled (`consumeReorderData`) |
| TC-HIS-03 | `alice` | Cancel a cancellable order | Order → cancelled; delivered/ready orders not cancellable |
...
| ID | Precondition | Steps | Expected |
|----|--------------|-------|----------|
| TC-DASH-01 | `baker_jean` | Login → `/dashboard` | Overview with KPI cards; data for owned bakery |
...
| TC-DASH-17 | `baker_jean` owns 2 bakeries | Switch active bakery (if selector present) | Data scopes to selected bakery |
| TC-DASH-18 | `baker_marie` | Try to view/mutate `bakery-1` orders (not hers) via API | Backend ownership check blocks (verifyBakeryOwnership) |
...
- Run against **in-memory backend** with seed data (fixtures in `e2e/helpers/test-data.ts` assume `alice`/`baker_jean`/`bakery-1`), OR add Postgres seeding — pick one and make CI match (MA-67).
