...
- [x] 1. Verify and fix ThemeSwitcher tests
  - [x] 1.1 Run `npx vitest run` in `frontend/` and confirm ThemeSwitcher tests pass
  ...
  - [x] 1.2 Confirm full frontend suite is green (226+ tests, 0 failures)
    - Run `npx vitest run` and verify exit code 0
    - Record pass/fail counts
    - _Requirements: 1.1_

- [x] 2. Fix CI workflow health check and verify E2E wiring
  - [x] 2.1 Audit `.github/workflows/ci.yml` e2e job for correctness
  ...
  - [x] 2.2 Verify E2E strategy coherence
...
- [x] 3. Fix bundle-utils property test (flaky counterexample)
  - [x] 3.1 Verify `calculateBundlePrice` rounding behavior and fix test invariant
...
- [x] 4. Checkpoint — Frontend suite fully green
  - Ensure all tests pass (`npx vitest run`), ask the user if questions arise.

- [x] 5. Verify and fix test theater issues
  - [x] 5.1 Verify `full-journey.spec.ts` has real assertions at every step
    ...
    - _Requirements: 4.1_
  - [x] 5.2 Verify StockStepper property tests render the actual component
    ...
    - _Requirements: 4.2_
  - [x] 5.3 Verify DashboardBundles test imports the correct routed component
    ...
    - _Requirements: 4.3_
  - [x] 5.4 Verify refund test has spy gateway assertion
...
- [x] 6. Verify E2E portal coverage exists
  - [x] 6.1 Confirm Comptoir portal E2E spec (`e2e/tests/comptoir.spec.ts`)
  ...
  - [x] 6.2 Confirm Baker portal E2E spec (`e2e/tests/baker-portal.spec.ts`)
...
- [x] 7. Final verification checkpoint
  - [x] 7.1 Run full frontend suite and confirm green
    - `cd frontend && npx vitest run` — expect 0 failures
    - _Requirements: 1.1_
  - [x] 7.2 Run Go test suite and confirm green
    - `go test -race ./...` — expect all pass including refund spy test
    - _Requirements: 4.4_
  - [x] 7.3 Verify CI YAML is syntactically valid
