...
- [ ] 1. Fix JWT base64url decode and add expiry check
  - [x] 1.1 Refactor `decodeTokenRole` in `frontend/src/api/client.ts` to use a new `decodeBase64Url` helper that translates `-`/`_` to `+`/`/` and pads with `=` before calling `atob()`; extract a `decodeTokenPayload` function returning the full payload object or null
