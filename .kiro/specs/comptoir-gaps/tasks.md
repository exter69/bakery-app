...
- [x] 1. Fix French accent-stripped i18n strings
  - [x] 1.1 Correct all accent-stripped FR strings in `frontend/src/i18n/translations.ts` comptoir section
  ...
  - [x] 1.2 Add `comptoir.deliveries.itemCount` i18n key for EN, FR, and NL
    - EN: `"{n} items"`, FR: `"{n} articles"`, NL: `"{n} artikelen"`
    - _Requirements: 4.5_
  - [x] 1.3 Replace hardcoded `{d.items.length} items` in `LivraisonsPage.tsx` with `t('comptoir.deliveries.itemCount', { n: d.items.length })`
    - Both the upcoming and past tables have this hardcoded string
    - _Requirements: 4.4_

- [x] 2. Create shared ErrorState component
  - [x] 2.1 Create `frontend/src/components/ErrorState.tsx`
...
- [x] 3. Fix swallowed errors on FacturesPage
  - [x] 3.1 Add error state to `FacturesPage.tsx`
    ...
    - _Requirements: 3.1_
  - [x] 3.2 Fix silent PDF download failure in `FacturesPage.tsx`
...
- [x] 4. Fix swallowed errors on LivraisonsPage
  - [x] 4.1 Add error state to `LivraisonsPage.tsx`
    - Add `const [error, setError] = useState<string | null>(null)` state
...
- [x] 5. Wire the Editer button on LivraisonsPage
  - [x] 5.1 Add onClick handler to the "Editer" button in `LivraisonsPage.tsx`
...
- [x] 6. Checkpoint - Verify error states and i18n
...
- [x] 7. Wire RecurrencesPage to the recurring-orders API
  - [x] 7.1 Add `createRecurringOrder` function to `frontend/src/api/recurring.ts`
    ...
    - _Requirements: 1.3_
  - [x] 7.2 Rewrite `RecurrencesPage.tsx` data fetching
  ...
  - [x] 7.3 Implement the creation form in `RecurrencesPage.tsx`
  ...
  - [x] 7.4 Implement pause/resume toggle in `RecurrencesPage.tsx`
  ...
  - [x] 7.5 Implement delete action in `RecurrencesPage.tsx`
    - Add delete button with confirmation prompt
    - Call `deleteRecurringOrder(id)` and remove row on success
    - _Requirements: 1.6_

- [x] 8. Add i18n keys for new RecurrencesPage and error UI strings
  - [x] 8.1 Add missing i18n keys for the creation form, error messages, and edit modal
...
- [x] 9. Final checkpoint - Full compilation and review
